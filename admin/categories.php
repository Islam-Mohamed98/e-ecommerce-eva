<?php

    /*
    ==================================
    ==
        Categories Page
    ==
    ==================================
    */

    ob_start(); // Out Put Buffering Start

    session_start();

    $pageTitle = 'Categories';

    if (isset($_SESSION['Username'])) {

        include 'init.php';

        $do = isset($_GET['do']) ? $_GET['do'] : 'Manage'; ?>

        <div class="dashboard-main" data-class="category" style="margin-left:15%;padding:50px 16px;">
        <?php

/////////////////////////////////////==[Manage]==///////////////////////////////////////


        if ($do == 'Manage') {

        // Get Data From categories table
        $sort = 'ASC';

        $sort_array = array('ASC ', 'DESC');

            if (isset($_GET['sort']) && in_array($_GET['sort'], $sort_array)) {

                $sort = $_GET['sort'];
            }

        $event = "SELECT * FROM catagory ORDER BY ID $sort ";

        $cats = sqlGlobal ($event, $fetch='fetchAll');

        ?>
        <h1 class="text-center">Manage Categories</h1>
        <div class="container categories">
                <div class="panel panel-default">
                    <div class="panel panel-heading">
                        <i class="fa fa-edit"></i> Manage Categories
                        <div class="option pull-right">
                            <i class="fa fa-sort"></i> Ordereing:[
                            <a class="<?php //if($sort == 'Asc'){echo 'active';}?>"href="?sort=ASC">Asc </a>
                            |
                            <a class="<?php //if($sort == 'Desc'){echo 'active';}?>"href="?sort=DESC"> Desc</a> ]
                            <i class="fa fa-eye"></i> View :[
                            <span class ="active" data-view="full" >Full</span> |
                            <span data-view="classic">Classic</span> ]
                        </div>
                    </div>
                    <div class="panel-body">
                        <?php
                            foreach($cats as $cat){
                                echo '<div class="cat">';
                                    echo '<div class="hidden-buttons">';
                                        echo '<a href="categories.php?do=Edit&catid=' .$cat['ID'] .'" class="btn btn-xs btn-primary"><i class="fa fa-edit"></i>Edit</a>';
                                        echo '<a href="categories.php?do=Delete&catid=' .$cat['ID'] .'" class="btn btn-xs btn-danger check"><i class="fa fa-times"></i>Delete</a>';
                                    echo '</div>';
                                    echo '<h3>' . $cat['Name'] . '</h3>';
                                echo '</div>';
                                echo '<hr>';

                            }
                        ?>
                    </div>
                </div>
            <a class="add-category btn btn-primary" href="categories.php?do=Add"><i class="fa fa-plus"></i> Add New Category</a>
        </div>


        <?php


 /////////////////////////////////////==[Add]==///////////////////////////////////////


        }else if ($do =='Add'){ ?>

            <!-- Start Html -->

                <h1 class="text-center">Add New Category</h1>
                <div class="container">
                    <form class="form-horizontal" action="?do=Insert" method="POST">

                        <!-- Start Name Filed -->
                        <div class="form-group">
                            <label class="col-sm-2 control-label">Name</label>
                            <div class="col-sm-10 col-md-4">
                                <input type="text" name="name"  class="form-control" autocomplete="off" required="required" placeholder="Name Of The Category" />
                            </div>
                        </div>
                        <!-- End Name Filed -->

                        <!-- Start Categories Filed -->
                        <div class="form-group">
                            <label class="col-sm-2 control-label">Sections</label>
                            <div class="col-sm-10 col-md-4">
                                <select class ="form-control" name="section">
                                    <option value ="0">...</option>
                                    <?php

                                        $stmt2 = $con->prepare("SELECT * FROM section ");
                                        $stmt2->execute();
                                        $sections = $stmt2->fetchAll();
                                        foreach($sections as $section) {
                                            echo'<option value ="' . $section['section_ID'].'">' . $section['Name']. '</option>';
                                        }

                                    ?>
                                </select>
                            </div>
                        </div>
                        <!-- End Categories Filed -->


                        <!-- Start Submit Filed -->
                        <div class="form-group">
                            <div class="col-sm-offset-2 col-sm-10 col-md-4">
                                <input type="submit" value="Add Category" class="btn btn-primary" />
                            </div>
                        </div>
                        <!-- End Submit Filed -->
                    </form>
                </div>

                <!-- End Html -->
        <?php


 /////////////////////////////////////==[Insert]==///////////////////////////////////////


        } else if ($do == 'Insert') {

            if ($_SERVER['REQUEST_METHOD'] == 'POST') {

                echo '<h1 class="text-center">Inseret Category</h1>';
                    echo '<div class="container">';

                // Get Variables From The Form

                $name             = $_POST['name'];
                $sectionsid       = $_POST['section'];
                //$desc       = $_POST['description'];
                //$order      = $_POST['ordering'];
                //$visible    = $_POST['visibility'];
                //$comment    = $_POST['commenting'];
                //$ads        = $_POST['ads'];
                //$parent     = $_POST['parent'];





                // Check If Category Exist In DataBase

                $check = checkItem('Name', 'catagory', $name);


                if ($check === 0) {

                    // Insert CATEGORY In DataBase

                    $stmt = $con->prepare("INSERT INTO catagory(Name, section_ID)
                                                      VALUES(:zname, :zsectionid)");
                    $stmt->execute(array(

                        'zname'          => $name,
                        'zsectionid'     => $sectionsid

                    ));

                    // Echo Sucess Message If Username Is Not Exist

                    $theMsg = '<div class="alert alert-success">' . $stmt->rowCount(). ' Recored Inserted</div>';

                    redirectFunction($theMsg, 'back');

                }else {

                    // Error  Message  When Category Exist

                    $theMsg = '<div class="alert alert-danger">This Category Already Exist</div>';

                    redirectFunction($theMsg, 'back');
                }




            } else {

                $errorMsg = '<div class="alert alert-danger">You Cant Open This Page Direct</div>';
                redirectFunction($errorMsg);
            }

            echo '</div>';


/////////////////////////////////////==[Edit]==///////////////////////////////////////


        } else if ($do == 'Edit') {

            // Check If GEt Request CatID Numeric & Get The Intehar Value Of It

            $catid = isset($_GET['catid']) && is_numeric($_GET['catid']) ? intval($_GET['catid']) : 0;

            // Select All Data Depend On This ID

            $stmt = $con->prepare("SELECT
                                        *
                                    FROM
                                        catagory
                                    WHERE
                                        ID = ? ");


            // Execute Query

            $stmt->execute(array($catid));

            // Fetch The Data

            $cat = $stmt->fetch();

            // The Row Count

            $count = $stmt->rowCount();

            // If There is Such ID Show The Form

            if($count > 0) { ?>

               <!-- Start Html -->

                <h1 class="text-center">Edit Category</h1>
                <div class="container">
                    <form class="form-horizontal" action="?do=Update" method="POST">
                        <input type="hidden" name="catid" value="<?php echo $cat['ID']?>" />
                        <!-- Start Name Filed -->
                        <div class="form-group">
                            <label class="col-sm-2 control-label">Name</label>
                            <div class="col-sm-10 col-md-4">
                                <input type="text" name="name"  class="form-control"  required="required" placeholder="Name Of The Category" value="<?php echo $cat['Name']; ?>" />
                            </div>
                        </div>
                        <!-- End Name Filed -->


                        <!-- Start Categories Filed -->
                        <div class="form-group">
                            <label class="col-sm-2 control-label">sections</label>
                            <div class="col-sm-10 col-md-4">
                                <select class ="form-control" name="section">
                                    <?php
                                        $stmt2 = $con->prepare("SELECT * FROM section ");
                                        $stmt2->execute();
                                        $sections = $stmt2->fetchAll();
                                        foreach($sections as $section) {
                                            echo'<option value ="' . $section['section_ID'].'"';
                                            if ($cat['section_ID'] == $section['section_ID']){echo 'selected';}
                                            echo'>' . $section['Name']. '</option>';
                                        }
                                    ?>
                                </select>
                            </div>
                        </div>
                        <!-- Start Submit Filed -->

                        <!-- Start Submit Filed -->
                        <div class="form-group">
                            <div class="col-sm-offset-2 col-sm-10 col-md-4">
                                <input type="submit" value="Save Category" class="btn btn-primary" />
                            </div>
                        </div>
                        <!-- End Submit Filed -->
                    </form>
                </div>

                <!-- End Html -->


<?php
            } else { // Else Show Error Message



                $theMsg = '<div class="alert alert-danger">' . $stmt->rowCount(). ' RThere Is No Such ID</div>';

                redirectFunction($theMsg);
            }


 /////////////////////////////////////==[update]==///////////////////////////////////////


        } else if ($do == 'Update'){

            echo '<h1 class="text-center">Update Category</h1>';
            echo '<div class="container">';

            if ($_SERVER['REQUEST_METHOD'] == 'POST') {

                $id         = $_POST['catid'];
                $name       = $_POST['name'];
                $sectionid  = $_POST['section'];
                //$desc       = $_POST['description'];
                //$order      = $_POST['ordering'];
                //$visiable   = $_POST['visibility'];
                //$comment    = $_POST['commenting'];
                //$ads        = $_POST['ads'];



                $stmt = $con->prepare("UPDATE
                                        catagory
                                    SET
                                        Name = ? ,
                                        section_ID = ?
                                    WHERE
                                        ID = ?");

                $stmt->execute(array($name, $sectionid, $id));

                // Echo Sucess Message

                $theMsg = '<div class="alert alert-success">' . $stmt->rowCount(). ' Recored Updated</div>';

                redirectFunction($theMsg, 'back');



            } else {


                $theMsg = '<div class="alert alert-danger">' . $stmt->rowCount(). ' You Cant Open This Page Direct</div>';

                redirectFunction($theMsg);
            }

            echo '</div>';



/////////////////////////////////////==[Delete]==///////////////////////////////////////


        } else if ($do == 'Delete') {

            echo '<h1 class="text-center">Delete Category</h1>';

            echo '<div class="container">';

            // Check If GEt Request userid Numeric & Get The Intehar Value Of It

            $catid = isset($_GET['catid']) && is_numeric($_GET['catid']) ? intval($_GET['catid']) : 0;

            // Select All Data Depend On This ID

             $check = checkItem('ID', 'catagory', $catid);

            // If There is Such ID Show The Form

            if($check > 0) {

                $stmt = $con->prepare("DELETE FROM catagory WHERE ID = :zcatid");

                $stmt->bindParam(':zcatid', $catid);

                $stmt->execute();

                $theMsg = '<div class="alert alert-success"> ID '.$catid.' Deleted</div>';

                redirectFunction($theMsg, 'back');

            } else {

                $theMsg = '<div class="alert alert-danger">' . $stmt->rowCount(). ' This Is ID is Not Exist</div>';

                redirectFunction($theMsg);
            }

            echo '</div>';


        }
/////////////////////////////////////////////////////////////////////////////////////////


        include $tpl . 'footer.php';

    } else {

        header('location: index.php');
        exit();
    }

ob_end_flush();

?>
